...
为了便于玩家添加通用乐器资源，模组工具[MFPackager](https://github.com/Seikai-Takenawa/MFPackager)已于GitHub上线。此工具可自动打包指定路径下的资源为模组可识别的资源包，并对通用乐器自动生成子乐器的注册json文件。
...
在模组中，MIDI文件的播放通过模组方块实体：MIDI音乐盒实现。其界面如下图所示：

![MIDI音乐盒界面](example.midi.screen.png)

MIDI音乐盒界面有三个选择栏目，从左到右分别为：
...
请注意：如果没有选任何乐器、轨道就点击播放，MIDI音乐盒会将相关信息设置为空。

### 停止
此按钮用于立即停止当前MIDI音乐盒的播放，并清空之前的演奏乐器、演奏轨道等播放信息。如果不点击此按钮，在MIDI序列正常播放结束后，其信息不会丢失。点击此按钮后，需要重新设定MIDI音乐盒。
...
### 结束控制按钮
此按钮默认显示为红色，表示当前音乐盒实体已被你占用。点击此按钮，你失去对此音乐盒实体的占有，播放界面将关闭，此时其它玩家可以右键打开播放界面并使用此音乐盒。

## MIDI音乐盒的控制

MIDI音乐盒支持使用红石控制，其与普通方块一样可以传递红石信号，但其本身并非红石方块。

MIDI音乐盒接收红石脉冲信号，在接收到一次脉冲信号后，会改变一次播放状态。

单个MIDI音乐盒在多人游戏模式下全服共用一个实体。为此，为了避免不同客户端使用冲突，MIDI音乐盒添加了玩家绑定。在初次放置后，会自动绑定首次打开播放界面的玩家。
在此玩家主动释放或方块被破坏并重新放置前，服务器内的其它玩家均无法右键打开播放界面，但是依然可以通过红石控制音乐盒的播放状态。

自定义音源资源包
=======
如果你不喜欢模组某个乐器的默认音源，且你有条件和能力获得音源如自行进行乐器采样，你可以自行替换相关资源包中的音源文件。

模组音源使用ogg格式，ogg文件要求（如音符持续时间、音符命名格式等）已在上方的[音符设定](#音符设定)中已详细说明，在此不再赘述。

安装
=======
模组提供两个安装版本：完整版和轻量版。

完整版：包含模组所有的资源文件，资源文件不可替换，体积较大，适合不愿意折腾的玩家。

轻量版：不包含模组中乐器的声音文件，需手动加载指定资源包才能演奏乐器，模组文件体积极小，适合服务端或想要定制化客户端乐器声音的玩家。需要注意的是：如果没有按模组资源格式添加对应乐器的资源包，则演奏乐器或者其它玩家同步MIDI音乐盒音符时将不会发出对应乐器的声音。
...
- 模组的功能性行为，如：模型替换、动画替换、通过修改资源包的形式修改模组中的乐器音源等。
- 你可以录制模组游戏内容，并在社交媒体上分享游戏内容。
- 你可以将本模组的副本分享给你的朋友，仅限个人、非商业用途。
- 你可以在你的 Minecraft 客户端或非商用服务器中使用本模组。

禁止的行为：

- 禁止除上方说明的允许修改的其它修改行为、反编译、反汇编或创建本模组的衍生作品。
- 禁止在其它任何网站、平台公开发布或重新上传本模组。
- 禁止将本模组用于任何商业目的（包括但不限于出售、作为付费整合包/服务器的一部分等）。
- 其它未明确说明允许的行为。
...
Q5.为什么小提琴没有琴弓？<br>
A5.小提琴加琴弓会对小提琴的使用和演奏逻辑产生影响，也使得演奏小提琴时模型的动画更加复杂。本人美术技术力有限实在不知道该怎么做这部分。（其实还有一个原因是懒，总之大家先当普通乐器演奏凑合凑合吧﹙ˊ_>ˋ﹚）

Q6.MIDI音乐盒速度不对/卡顿/变快。<br>
A6.由于模组使用我的世界内置tick系统计时，其精准度受到服务端/客户端的tick值波动的影响。

Q7.较低版本会适配吗？将来会有Forge/Fabric端的模组吗？<br>
