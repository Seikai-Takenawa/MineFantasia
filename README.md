...
自定义玩家模型
=======
为了让模组的乐器演奏系统更加生动，模组添加了基于GeckoLib5的动画系统，并将玩家模型替换为了GeckoLib模型。<br>

模型支持替换，但ysm模型或其它模组的模型在此模组中不受支持。你应该使用GeckoLib模型。<br>
在你初次运行模组并进入世界后，会在你的`mods`文件夹下生成模组的专属文件夹。其文件结构为：<br>
<details style="background-color: #d4d4d433; border-radius: 8px; padding: 15px; border: 1px solid #e1e4e8;">
...
其中，`model`文件夹存放你所有的GeckoLib模型文件`key.geo.json`，`animation`文件夹存放你所有的GeckoLib模型动画文件`key.animation.json`，`texture`文件夹存放你所有的GeckoLib模型贴图`key.png`。<br>

`uuid.json`文件是一个以玩家uuid命名的文件，其内容含有四个关键记录字段：<br>
<details style="background-color: #d4d4d433; border-radius: 8px; padding: 15px; border: 1px solid #e1e4e8;">
<summary style="cursor: pointer; font-weight: bold; color: #ffffffcc; font-size: 1.1em;">
 点击展开
</summary>
...
模组对模型骨骼数量及其结构并没有特别严格的规定和限制，但请保证在你的自定义GeckoLib模型中包含符合如下要求的重要骨骼：<br>
<details style="background-color: #d4d4d433; border-radius: 8px; padding: 15px; border: 1px solid #e1e4e8;">
<summary style="cursor: pointer; font-weight: bold; color: #ffffffcc; font-size: 1.1em;">
 点击展开
</summary>
...
自定义模型动画|Customize Model Animations
=======
模组支持自定义玩家模型动画。由于所有动画名称硬编码在模组中，所以你的自定义模型动画名称需与模组编码的动画名称相同。<br>
当前支持的动画及其命名要求如下：<br>
<details style="background-color: #d4d4d433; border-radius: 8px; padding: 15px; border: 1px solid #e1e4e8;">
<summary style="cursor: pointer; font-weight: bold; color: #ffffffcc; font-size: 1.1em;">
 点击展开
...
=======
Q1.我该如何获取乐器？<br>
A1.目前，乐器暂不支持合成和制造。非创造模式下，模组的钢琴自然生成于模组结构音乐厅（Concert Hall）的中心位置且不可破坏，其它的乐器仅能够在音乐厅的后台箱子里获取。<br>

Q2.在多人游戏下，我没法听到其它玩家演奏乐器的声音。<br>
A2.多人游戏下，乐器演奏系统只会将演奏信息同步到与正在演奏乐器的那一位玩家处于同一区块内玩家。<br>
